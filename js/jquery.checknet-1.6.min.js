...
(function(b) {
	b.fn.checknet = function(a) {
		...
		function d(a) {
			window.checknet.windowIsActive && b.ajax({
				...
				window.checknet.conIsActive ? (b("#netWarn").length && b("#netWarn").fadeOut("fast", function() {
					b(this).remove()
				}), b(".tempDisabled").removeAttr("disabled").removeClass("tempDisabled")) : (b("#netWarn").length || (b("body").prepend('<p id="netWarn">' + window.checknet.config.warnMsg + "</p>"), b("#netWarn").fadeIn("fast")), b('input[type="submit"]').attr("disabled", "disabled").addClass("tempDisabled"))
			});
			setTimeout(function() {
		...
		a.warnMsg = a.msg || "No Internet connection detected, disabled features will be re-enabled when a connection is detected. ";
		-1 === a.checkURL.indexOf("http") && (a.checkURL = "http://" + a.checkURL);
		window.checknet = {
			windowIsActive : !0,
			conIsActive : !0,
			config : a
